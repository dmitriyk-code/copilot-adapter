...
- `docs/design/COPILOT-1M-MODEL-CONSOLIDATION.design.md` - Design document for dropping `-1m` / Opus 4.7 effort model-name suffixes after GitHub Copilot consolidated its Claude SKUs (1M is now native to base models)
- `docs/design/COPILOT-1M-MODEL-CONSOLIDATION.plan.md` - Implementation plan for the Copilot 1M model consolidation change
...
- **1M context handling**: When Claude Code's user selects a "1M context" model variant, Claude Code sends `anthropic-beta: context-1m-*` as an HTTP header (not in the JSON body) and strips `[1m]` from the model name before sending. The adapter's `has_1m_context_beta()` function in `src/handlers/messages.rs` detects this header (prefix match on `context-1m`) and logs it for diagnostics, but **does not modify the model name**. GitHub Copilot consolidated its Claude SKUs: the live `/models` list exposes only base names (e.g., `claude-opus-4.6`, `claude-opus-4.7`) with no `-1m` / `-1m-internal` IDs, and those base models are 1M-native (Opus 4.6/4.7/4.8 and Sonnet 4.6 per Anthropic; 1M is a context-size toggle per the Copilot docs). Appending `-1m` would now select a non-existent model, so the adapter forwards the normalized base name unchanged. The `HeaderMap` is extracted alongside `Json<AnthropicRequest>` in the messages handler. See `docs/design/COPILOT-1M-MODEL-CONSOLIDATION.design.md`. (`normalize_model_name()` still *preserves* an in-name `-1m` marker for hypothetical direct API callers — Claude Code never sends one.)
- **Effort and thinking support**: The adapter translates Claude Code's effort and thinking parameters to the OpenAI/Copilot format. `AnthropicRequest` accepts `output_config: Option<OutputConfig>` (with `effort: Option<String>`) and `thinking: Option<serde_json::Value>` fields in `src/anthropic/types.rs`. During `to_chat_completion_request()`, `output_config.effort` is translated to `reasoning: Option<Reasoning>` in `ChatCompletionRequest` (`src/copilot/types.rs`), with `"max"` mapped to `"high"` (other values, including `"xhigh"`, passed through). This applies uniformly to **all** Claude models including Opus 4.7 — effort is no longer encoded as a model-name SKU suffix (`-high` / `-xhigh`), since Copilot consolidated those SKUs. When `thinking` is present, temperature is suppressed (`None`) to avoid API errors. `ContentBlock::Thinking` and `ContentBlock::RedactedThinking` variants are accepted during deserialization of conversation history but stripped by `strip_thinking_blocks()` before translation to the OpenAI format, since the Copilot API does not support thinking content blocks.
